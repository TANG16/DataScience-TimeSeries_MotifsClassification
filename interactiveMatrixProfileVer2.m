...
function [matrixProfile, profileIndex, motifIdxs, discordIdx,motifIdxs10] = ...
    interactiveMatrixProfileVer2(data, subLen)
%% options for the algorithm
excZoneLen = round(subLen * 0.5);
radius = 2;
updatePeriod = 1; % in second
anytimeMode = 2; % 1: original with  O(n^2 log n); 
...
for i = 1:length(idxOrder)
    ...
    % find matif
    [motifIdxs, matrixProfileCur,motifIdxs10] = findMotifs(...
...
function [motifIdxs, matrixProfileCur,motifIdxs10] = findMotifs(...
    matrixProfileCur, profileIndex, dataLen, subLen, proLen, ...
    data, dataFreq, dataMu, dataSig, isSkip, excZoneLen, radius)
motifIdxs = cell(10, 2); %%==> choose the number of motifs here
for i = 1:10 %==> and here, now chosen as 10
...
motifIdxs10=motifIdxs;
motifIdxs=motifIdxs10(1:3,:);
